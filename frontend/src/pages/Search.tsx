...
const Search = () => {
    ...
    return(
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-5">
            <div className="rounded-lg border border-slate-300 p-5 fit sticky top-10">
            ...
            <div className="flex flex-col gap-5">
                <div className="flex justify-between items-center">
                    ...
                    <SortFilter sortOption={sortOption} onChange={(option : string) => setSortOption(option)} />
                </div>
                {hotelData?.data.map((hotel) => (
                <SearchResultCard hotel = {hotel}/>
                ))}
                <div>
                    <Pagination page={hotelData?.pagination.page || 1} pages={hotelData?.pagination.totalPages || 1} onPageChange={(page) => setPage(page)}/>
                </div>
            </div>
        </div>
        </>
